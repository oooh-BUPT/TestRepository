...
# 构建训练集和验证集
# 
train_set = ImageFolder('D:/work/PatternRecognition/dataset/train/', train_transform)
train_data = DataLoader(train_set, batch_size, shuffle=True, num_workers=0)
 
valid_set = ImageFolder('D:/work/PatternRecognition/dataset/test/', test_transform)
...
model = get_model()
with torch.no_grad():
    scorce = model(iter(train_data).__next__()[0])
...
# 保存模型
torch.save(model.state_dict(), 'D:/work/PatternRecognition/save_model1.pth')